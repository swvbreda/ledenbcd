...
export default function FinancienPage() {
  ...
  const totalBudgeted = (categories || []).reduce(
    (s, c) => s + c.line_items.reduce((ls, li) => ls + li.budgeted_amount, 0),
    0
  );
  const totalSpent = (categories || []).reduce(
    (s, c) => s + c.line_items.reduce((ls, li) => ls + li.expenses.reduce((es, e) => es + e.amount, 0), 0),
    0
  );
  ...
  // Find all expenses for the selected line item
  const selectedLineItemExpenses = expenseDialog
    ? (categories || [])
        .flatMap((c) => c.line_items)
        .find((li) => li.id === expenseDialog.lineItemId)?.expenses || []
    : [];
          ...
          <div className="space-y-3">
            {(categories || []).map((cat) => (
              <BudgetCategoryTable
                ...
                onUpdateLineItem={(id, name, amount) =>
                  mutations.updateLineItem.mutate({ id, name, amount })
                }
                onDeleteLineItem={(id) =>
                  mutations.deleteLineItem.mutate(id, { onSuccess: () => toast.success("Post verwijderd") })
                }
                onDeleteCategory={(id) =>
                  mutations.deleteCategory.mutate(id, { onSuccess: () => toast.success("Categorie verwijderd") })
                }
                onOpenExpenses={(lineItemId, lineItemName) =>
                  setExpenseDialog({ lineItemId, lineItemName })
                }
              />
            ))}
                ...
                <table className="w-full text-sm">
                  <tbody>
                    <tr className="font-bold">
                      ...
                      <td className="text-right px-3 py-2 tabular-nums w-[18%]">
                        {new Intl.NumberFormat("nl-NL", { style: "currency", currency: "EUR" }).format(totalBudgeted)}
                      </td>
                      <td className="text-right px-3 py-2 tabular-nums w-[18%]">
                        {new Intl.NumberFormat("nl-NL", { style: "currency", currency: "EUR" }).format(totalSpent)}
                      </td>
                      <td className={`text-right px-3 py-2 tabular-nums w-[18%] ${totalBudgeted - totalSpent < 0 ? "text-destructive" : "text-green-600"}`}>
                        {new Intl.NumberFormat("nl-NL", { style: "currency", currency: "EUR" }).format(totalBudgeted - totalSpent)}
                      </td>
                      <td className="w-[6%]" />
                    </tr>
                  </tbody>
          ...
          <div>
            <BalancePanel
              ...
              onAdd={(name, amount, section) =>
                mutations.addBalanceItem.mutate({ name, amount, section }, {
                  onSuccess: () => toast.success("Post toegevoegd"),
                })
              }
      ...
      {expenseDialog && user && (
        <ExpenseDialog
          ...
          onAddExpense={(expense) =>
            mutations.addExpense.mutate(expense, { onSuccess: () => toast.success("Uitgave toegevoegd") })
          }
          onDeleteExpense={(id) =>
            mutations.deleteExpense.mutate(id, { onSuccess: () => toast.success("Uitgave verwijderd") })
          }
          userId={user.id}
        />
      )}
