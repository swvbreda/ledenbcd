  ...
  return (
    <>
      <Sidebar collapsible="icon">
        <SidebarHeader
          className="border-b border-border px-3 py-0 bg-white h-[calc(5rem+env(safe-area-inset-top))] pt-[env(safe-area-inset-top)] flex flex-row items-center cursor-pointer"
          onClick={() => {
            navigate("/");
            closeMobile();
          }}
        >
          {!collapsed ? (
            <img
              src={bcdLogo}
              alt="Bond van Cannabis Detaillisten"
            ...
            <SidebarGroupContent>
              <SidebarMenu>
                {navItems.map((item) => {
                  ...
                  const registerItem = canSeeRegister &&
                    item.title === "Ledenbestand" && (
                      <>
        ...
        <SidebarFooter className="border-t border-sidebar-border px-4 py-3 space-y-2">
          {!collapsed && user && (
            <div className="flex items-center gap-1.5">
              {isAdmin && (
                <Shield size={12} className="text-brand-red shrink-0" />
              )}
