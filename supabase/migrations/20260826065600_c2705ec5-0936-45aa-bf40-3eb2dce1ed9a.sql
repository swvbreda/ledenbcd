...
fallback_locations AS (
  SELECT em.member_id, gs.n AS location_index, COALESCE(NULLIF(btrim(em.edit_data->>'plaats'), ''), NULLIF(btrim(em.data->>'plaats'), ''), 'Onbekend') AS gemeente, ''::text AS location_key, ''::text AS postcode
