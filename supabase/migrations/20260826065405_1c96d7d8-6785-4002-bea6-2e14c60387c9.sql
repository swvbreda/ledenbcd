...
confirmed AS (
  SELECT DISTINCT ON (l.register_id)
    l.register_id, l.member_id, NULLIF(btrim(l.location_key), '') AS location_key, ar.gemeente
...
effective_members AS (
  SELECT md.id AS member_id, md.data, me.data AS edit_data
...
member_locations AS (
  SELECT
    ...
    COALESCE(NULLIF(btrim(loc.value->>'plaats'), ''), NULLIF(btrim(em.edit_data->>'plaats'), ''), NULLIF(btrim(em.data->>'plaats'), '')) AS gemeente,
    lower(regexp_replace(COALESCE(loc.value->>'naam', ''), '[^a-zA-Z0-9]', '', 'g')) || '|' ||
      lower(regexp_replace(COALESCE(loc.value->>'adres', ''), '[^a-zA-Z0-9]', '', 'g')) || '|' ||
      lower(regexp_replace(COALESCE(loc.value->>'postcode', ''), '[^a-zA-Z0-9]', '', 'g')) AS location_key,
...
fallback_locations AS (
  SELECT
    ...
    COALESCE(NULLIF(btrim(em.edit_data->>'plaats'), ''), NULLIF(btrim(em.data->>'plaats'), '')) AS gemeente,
    ''::text AS location_key,
    ''::text AS postcode
  FROM effective_members em
  CROSS JOIN LATERAL generate_series(1, GREATEST(COALESCE(NULLIF(COALESCE(em.edit_data->>'aantalLocaties', em.data->>'aantalLocaties'), '')::integer, 1), 1)) AS gs(n)
  WHERE NOT EXISTS (
    SELECT 1
...
ranked_locations AS (
  SELECT ml.*, row_number() OVER (PARTITION BY ml.member_id ORDER BY ml.location_index) AS member_rank
  FROM all_locations ml
),
confirmed_counts AS (
  SELECT member_id, count(*)::integer AS link_count
  FROM confirmed
  GROUP BY member_id
),
  ...
  SELECT
    ...
    EXISTS (
      ...
      WHERE c.member_id = ml.member_id
        AND (c.location_key = ml.location_key OR upper(regexp_replace(c.location_key, '\s+', '', 'g')) = ml.postcode)
    )
    OR ml.member_rank <= COALESCE(cc.link_count, 0) AS has_confirmed_link
  FROM ranked_locations ml
  LEFT JOIN confirmed_counts cc ON cc.member_id = ml.member_id
),
represented AS (
  SELECT c.gemeente, 'register'::text AS source FROM confirmed c
  UNION ALL
  SELECT cl.gemeente, 'location'::text AS source FROM classified_locations cl WHERE NOT cl.has_confirmed_link
),
municipalities AS (
