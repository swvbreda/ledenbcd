-- Laagste vrije lidnummer. Historienummers (>= 10000) tellen niet mee.
...
declare
  _next integer;
begin
  if not (public.has_role(auth.uid(), 'admin') or public.is_board_member(auth.uid())) then
    raise exception 'Niet toegestaan';
  end if;

  -- Serialiseren zodat twee gelijktijdige aanmakingen nooit hetzelfde nummer krijgen.
...
revoke execute on function public.next_member_number() from public, anon;
grant execute on function public.next_member_number() to authenticated, service_role;

-- Verplaatst een gestopt lid naar een historienummer zodat het lidnummer vrijkomt.
...
begin
  ...
  if _member_id >= 10000 then
    return _member_id; -- al een historienummer
  end if;
  ...
  -- 1. Nieuw archiefrecord met het historienummer
  insert into members_data (id, member_type, data)
  values (_new_id, 'old', jsonb_set(_data, '{id}', to_jsonb(_new_id)));

  -- 2. Alle verwijzingen meeverhuizen
  foreach _t slice 0 in array _pairs loop
    null;
  end loop;
  ...
  -- 3. Oud record verwijderen; het lidnummer is nu weer vrij
...
revoke execute on function public.archive_member_with_renumber(integer) from public, anon;
grant execute on function public.archive_member_with_renumber(integer) to authenticated, service_role;