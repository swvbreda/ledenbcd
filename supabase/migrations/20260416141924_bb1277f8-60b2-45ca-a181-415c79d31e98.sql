...
BEGIN
  ...
  FROM (
    SELECT
      ...
      COALESCE(
        NULLIF(me.data->>'contactpersoon', ''),
        NULLIF(md.data->>'contactpersoon', ''),
        (
          SELECT contact->>'naam'
          FROM jsonb_array_elements(COALESCE(me.data->'contacten', md.data->'contacten', '[]'::jsonb)) WITH ORDINALITY AS contacts(contact, ord)
          WHERE COALESCE(contact->>'naam', '') <> ''
          ORDER BY ord
          LIMIT 1
        )
      ) AS contactpersoon,
