...
const STEPS: string[] = [
  ...
  'Klik onderaan op "Nog geen account? Registreren"',
  ...
  'Controleer daarna je gegevens: contactpersoon, adres, locaties en factuurgegevens',
]

const MemberWelcomeStepsEmail = ({ subject, body, portalUrl, showSteps = true }: Props) => {
  const url = portalUrl || 'https://leden.coffeeshopbond.nl'
  const paragraphs = (body || '').split(/\n{2,}/).filter((p) => p.trim().length > 0)
  return (
    <Html lang="nl" dir="ltr">
      <Head />
      <Preview>{subject || 'Welkom bij de Bond van Cannabis Detaillisten'}</Preview>
      <Body style={main}>
        <Container style={container}>
          ...
          <Heading style={h1}>{subject || 'Welkom bij de Bond van Cannabis Detaillisten'}</Heading>
          ...
          {showSteps ? (
            <>
              ...
              <Section style={card}>
                <Text style={cardTitle}>Zo maak je je account aan</Text>
                {STEPS.map((s, i) => (
              ...
              <Section style={waCard}>
                <Text style={cardTitle}>WhatsApp-community</Text>
                <Text style={step}>
                  Zodra je bent ingelogd, vind je in het menu van het ledenportaal de link naar de
                  WhatsApp-community van de BCD. Klik daarop om deel te nemen — de uitnodigingslink
                  staat bewust alleen achter de login.
                </Text>
              </Section>
            </>
          ) : null}
          ...
          <Text style={help}>
            ...
            met je naam en coffeeshop, dan helpen wij je verder.
          </Text>
...
export const template = {
  component: MemberWelcomeStepsEmail,
  subject: (data: Record<string, any>) => data.subject || 'Welkom bij de Bond van Cannabis Detaillisten',
  displayName: 'Welkomstmail met inloguitleg',
  previewData: {
    subject: 'Welkom bij de Bond van Cannabis Detaillisten',
    body:
      'Beste contactpersoon,\n\nWelkom bij de BCD! Coffeeshop Voorbeeld in Amsterdam is toegevoegd aan ons ledenbestand.\n\nHieronder lees je hoe je toegang krijgt tot het ledenportaal.',
    showSteps: true,
  },
